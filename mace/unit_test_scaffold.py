...
def scaffold_env(piton_root: str, env_name: str, dv_root: str | None = None) -> dict:
    ...
    if env_dir.is_dir():
        return {
            ...
            "env_dir": str(env_dir),
            "top_v": str(top_v),
            "flist": str(env_dir / f"{env_name}.flist"),
            "config": str(dv / "tools" / "src" / "sims" / f"{env_name}.config"),
        }
