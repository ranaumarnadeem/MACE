...
under Verilator. We also traced what kept 2x2 and 4x4 meshes from passing, including a Verilator~5
scheduling bug that makes CVA6's cache adapter drop invalidations; with those issues addressed,
MACE's loop passes both sizes for Ariane and PicoRV32. The one-shot baseline failed on all three cores in three different ways; MACE
...
\textbf{Three approaches, three cores.} We ran all three approaches on the same objective, verifying
\code{barrier\_atomic.c}, with Gemini~2.5~Flash throughout (Table~\ref{tab:eval}).
...
(b) fails in several distinct ways. On ariane at 1x1, the one-shot config built but failed
verification: a wrong guess. On sparc, it built and reached a run attempt that failed silently, with
no verdict at all. On pico, and on ariane at 2x2, \code{PitonConfig}'s construction-time validation
rejected the LLM's config (\code{l15\_size=0}) before any build; (b) has no retry path for a
malformed response. At 4x4 it chose oversized caches, including a 4\,MB L2 per tile: the build took
35 minutes and the simulation timed out with no tile finished.
...
Verilator~5.020 (with \code{--no-timing}, which Verilator~5 requires here), while CI itself uses
Verilator~4, so the stall is environmental rather than caused by \sys{}; its root cause remains open.
...
\$0.30 and \$2.50 per million input and output tokens. \sys{}'s own cost counter records nothing:
CHIA's Vertex backend reports no price and omits thinking tokens, which are more than half of the
billed output here, so we measured tokens by re-sending representative planner and task prompts.
