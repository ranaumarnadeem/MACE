...
`scripts/patch_openpiton.sh` applies twelve numbered fixes to an OpenPiton checkout and adds one unit-test environment. Each fix edits a file in the checkout. The boot ROM `Makefile`, for example, sets its compiler flags with plain `=` assignments, so neither the environment nor a `sims` flag can override them.
...
Run it once per checkout, on every worker that hosts one. The GCP worker in `cluster/local.yaml` runs it from `worker_setup_commands`. The script needs `bash`, `git`, and `python3`. It exits with status 2 when no root is given or the root has no `piton/` directory. It exits with status 1 when the boot ROM `Makefile` is missing, when a text block or anchor it edits does not appear exactly once, or when fix 5 finds a half-applied guard.
...
| 2 | Same `Makefile` | GCC 15 and later default to C23, where `void init_uart();` declares a function with no arguments, so the boot ROM's two-argument call fails. Appends `-std=gnu17` to `CFLAGS`. The boot ROM's `clean` target keeps `.o` files, so a stale `main.o` can hide this failure. |
...
The environment builds under Verilator, using fix 8. Running it, or any other non-manycore testbench such as OpenPiton's `ifu_esl_lfsr`, stops with Verilator's `Settle region did not converge` error. Three macros in the shared `piton/verif/env/test_infrstrct/test_infrstrct.v` harness place delays inside `always @*` blocks.
