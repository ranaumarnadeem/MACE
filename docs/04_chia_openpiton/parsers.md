...
| `coverage_summary` | `{"hit": 8749, "total": 24311, "percent": 35.99}` from the `verilator_coverage --annotate` line `Total coverage (8749/24311) 35.00%`. The percent comes from the counts, because `verilator_coverage` prints it rounded down to a whole number. All three values are `None` when the line is missing, so a failed `verilator_coverage` run never reads as 0% |
