...
Ariane is OpenPiton's RV64GC core, built from the CVA6 repository that OpenPiton carries as a submodule. It has its own write-through L1 caches and reaches the rest of the tile through CVA6's L1.5 adapter, `core/cache_subsystem/wt_l15_adapter.sv`. MACE selects it with `core="ariane"`, which adds `-ariane` to the `sims` command line.
...
Three patches in `scripts/patch_openpiton.sh` are specific to multi-tile Ariane runs. See [Environment Patches](../04_chia_openpiton/environment_patches.md) for the full list.
...
Upstream CVA6 fixed this in [cva6#2809](https://github.com/openhwgroup/cva6/pull/2809), which moves those fields into the always block. OpenPiton pins CVA6 at commit `4c01614f8` (2022-10-13), before that fix. An A/B test on a clean 2x2 build with the unmodified `syscalls.c` isolates it:

| CVA6 adapter | Result |
|---|---|
| cva6#2809 ported by hand | pass, 4 of 4 tiles, 24s |
| Original, as OpenPiton pins it | timeout, 1 of 4 tiles, 431s |

Fix 11 is a workaround for this issue, and the published results use it. Porting cva6#2809 is the proper fix. It also explains why MACE's gate workloads read shared values through an `atomic_read()` helper.
...
- Verilator 5 needs `--no-timing` for this design.
