...
| Monitor tracking | `piton/verif/env/manycore/pc_cmp.v.pyv` (patch fix 7) | The monitor's `active_thread` bit for pico waited on the same interrupt. It now turns on after reset, as Ariane's block does, so the monitor detects good and bad traps. |
| BIST self-clear | RTL define `CONFIG_DISABLE_BIST_CLEAR` | OpenPiton's generic SRAM model (`bram_1rw_wrapper.v`) runs a power-on BIST clear. PicoRV32 boots faster than the other cores, so the clear discards its first memory writes. The define turns the clear off. |
